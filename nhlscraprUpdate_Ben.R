...
full.game.database <- function(extra.seasons = 0) {
    ...
    playoff.series.lengths <- c(
        ...
        6, 6, 6, 5, 4, 5, 4, 6,  6, 7, 6, 7,  7, 6,  6, #20162017 - bbzhang
...
getGameCodes <- function(season) {
  # Returns gcodes for a specific completed season
  playoff.series.lengths <- c(
    4, 4, 6, 7, 7, 7, 6, 4,  5, 7, 7, 5,  6, 6,  6, #20022003
    6, 5, 5, 6, 7, 6, 4, 7,  7, 6, 5, 6,  7, 7,  4, #20032004
    7, 5, 7, 5, 7, 6, 5, 7,  5, 4, 6, 6,  6, 7,  6, #20052006
  ...
  seasons <- c("20022003", "20032004", "20052006", "20062007", "20072008", "20082009", "20092010", "20102011", "20112012", "20122013", "20132014", "20142015",
               "20152016", "20162017")
  output <- matrix(unlist(playoff.series.lengths), ncol = 15, byrow = TRUE)
  rownames(output) <- seasons
  desired.playoff.series <- output[which(seasons == season),]
  
  gcodes <- c(1:1230)
  playoff.index <- 10000
  round.index <- 1
  round.series <- c(8,4,2,1)
  for (ii in 1:length(round.series)) {
    series.count <- round.series[ii]
    for (jj in 1:series.count) {
      gcodes <- c(gcodes,((ii*100+jj*10+1:desired.playoff.series[round.index])))
